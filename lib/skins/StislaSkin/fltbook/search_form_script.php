...
<div class='card-body'>
    <div class='alert alert-primary mb-2' role='alert'>
        <strong>糟糕！</strong>我们没有任何从您当前位置出发的航班！
    </div>
</div>
<?php } else { ?>
...
<script type="text/javascript">
    ...
    arrLatlng.forEach(function(d, i) {
        if(d.latitude != null && d.latitude != undefined) {
            ...
            var contentString = "<?php echo $route->arricao.' - '.$arrival->name.' '.$hub?>"+' | <form action="<?php echo url('/fltbook');?>" method="post" id="<?php echo $route->depicao; ?><?php echo $route->arricao; ?>" style="display: inline"><input id="depicao" name="depicao" type="hidden" value="<?php echo $route->depicao; ?>"><input id="airline" name="airline" type="hidden" value="<?php echo $airline->code; ?>"><input id="aircraft" name="aircraft" type="hidden" value=""><input id="arricao" name="arricao" type="hidden" value="<?php echo $route->arricao; ?>"><input type="hidden" name="action" value="search" /><a href="javascript::" onclick="document:<?php echo $route->depicao; ?><?php echo $route->arricao; ?>.submit()" class="m-link">预定航班</a></form>';
